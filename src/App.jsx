...
function App() {
  ...
  const handleAddCart = (product, quantity) => {
    setCartItems((prevItems) => {
      // se nao existir => adiciona o item
  ...
  const handleUpdateCart = (product, quantity) => {
    toast.info(`Quantidade de item ${product.name} atualizada!`);
    setCartItems((prevItems) => {
      return prevItems.map((item) => item.id === product.id ? {...item, quantity: +quantity}:
        item);
    });
  };
